...
public class LoginService {
	...
	public LoginMgt userLogin(LoginMgt loginmgt) {
		//Password는 암호화할것.
		...
		if(loginIdInfo.isEmpty()) {
			throw new NoticeBoardException("로그인정보가 없습니다.[0]", HttpStatus.INTERNAL_SERVER_ERROR);
		}
		...
		if(loginIdPwInfo.isEmpty()) {
			//비밀번호오류횟수 추가
			if(passErrorCnt > 4) {
				throw new NoticeBoardException("비밀번호오류횟수를 초과했습니다.비밀번호찾기페이지로 이동합니다." ,  HttpStatus.INTERNAL_SERVER_ERROR , "COMM_0001");
			}
			...
			throw new NoticeBoardException("비밀번호가 틀립니다.오류횟수["+(passErrorCnt+1)+"]" , HttpStatus.INTERNAL_SERVER_ERROR);
		}
